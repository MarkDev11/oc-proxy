...
// We follow upstream free tier: any `*-free` model is allowed and handled
// via the Responses endpoint (same as muse-spark). Hard-coded set kept as
...
function isAllowedModel(id) {
  const b = baseModel(id).toLowerCase();
  if (!b) return false;
  if (RESPONSES_MODELS.has(baseModel(id))) return true;
  // Follow upstream: any *-free model is allowed
  if (b.includes("free")) return true;
  return false;
}
...
/** Upstream rejects non-auto tool_choice with 400 on free models (verified for muse-spark, applied to all *-free to auto-follow upstream). */
function forceAuto(model, body) {
  const b = baseModel(model).toLowerCase();
  const needsAuto = FORCE_AUTO_TOOL_CHOICE.has(baseModel(model)) || b.includes("free");
  if ("tool_choice" in body && body.tool_choice !== "auto" && needsAuto) {
