...
function createTranslator(openaiModel) {
  ...
  const state = { text: "", tools: new Map(), order: [], usage: null, respModel: openaiModel };
  ...
  function finalChunk() {
    ...
    const finish = tools.length ? "tool_calls" : "stop";
    const extra = state.usage ? { usage: state.usage } : null;
    return chunk({}, finish, extra);
  }
  ...
  return { state, apply, finalChunk, finalJson };
}
